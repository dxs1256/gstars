...
> `#DevOps Automation` `#Native Development` `#Terminal Emulation` `#android` `#android-app` `#jetpack-compose` `#kotlin` `#mosh` `#ssh` `#ssh-client` `#terminal` `#terminal-emulator` `#tui` 
> Moke is a native Android SSH/mosh terminal built with Kotlin and Jetpack Compose. It supports multi-session management, mosh roaming, and full-screen TUIs like vim and Claude Code. Featuring Keystore-encrypted credentials, bilingual support, and advanced font management, it delivers a professional remote connection and DevOps experience on mobile devices.
